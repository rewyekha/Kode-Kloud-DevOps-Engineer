# Table of contents

* [Kode Kloud - k8s](README.md)
* [Day 1: Deploy Pods in Kubernetes Cluster](day-1-deploy-pods-in-kubernetes-cluster.md)
* [Day 2: Deploy Applications with Kubernetes Deployments](day-2-deploy-applications-with-kubernetes-deployments.md)
* [Day 4: Script Execution Permissions](day-4-script-execution-permissions.md)
* [Day 5: Execute Rolling Updates in Kubernetes](day-5-execute-rolling-updates-in-kubernetes.md)
* [Revert Deployment to Previous Version in Kubernetes](revert-deployment-to-previous-version-in-kubernetes.md)
* [Deploy ReplicaSet in Kubernetes Cluster](deploy-replicaset-in-kubernetes-cluster.md)
* [Schedule Cronjobs in Kubernetes](schedule-cronjobs-in-kubernetes.md)
* [Create Countdown Job in Kubernetes](create-countdown-job-in-kubernetes.md)
* [Set Up Time Check Pod in Kubernetes](set-up-time-check-pod-in-kubernetes.md)
* [Resolve Pod Deployment Issue](resolve-pod-deployment-issue.md)
* [Update Deployment and Service in Kubernetes](update-deployment-and-service-in-kubernetes.md)
