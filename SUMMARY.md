# Table of contents

## K8s

* [Kode Kloud - k8s](README.md)
* [Day 1: Deploy Pods in Kubernetes Cluster](k8s/day-1-deploy-pods-in-kubernetes-cluster.md)
* [Day 2: Deploy Applications with Kubernetes Deployments](k8s/day-2-deploy-applications-with-kubernetes-deployments.md)
* [Day 4: Script Execution Permissions](k8s/day-4-script-execution-permissions.md)
* [Day 5: Execute Rolling Updates in Kubernetes](k8s/day-5-execute-rolling-updates-in-kubernetes.md)
* [Revert Deployment to Previous Version in Kubernetes](k8s/revert-deployment-to-previous-version-in-kubernetes.md)
* [Deploy ReplicaSet in Kubernetes Cluster](k8s/deploy-replicaset-in-kubernetes-cluster.md)
* [Schedule Cronjobs in Kubernetes](k8s/schedule-cronjobs-in-kubernetes.md)
* [Create Countdown Job in Kubernetes](k8s/create-countdown-job-in-kubernetes.md)
* [Set Up Time Check Pod in Kubernetes](k8s/set-up-time-check-pod-in-kubernetes.md)
* [Resolve Pod Deployment Issue](k8s/resolve-pod-deployment-issue.md)
* [Update Deployment and Service in Kubernetes](k8s/update-deployment-and-service-in-kubernetes.md)
* [Deploy Highly Available Pods with ReplicationController](k8s/deploy-highly-available-pods-with-replicationcontroller.md)
* [Resolve VolumeMounts Issue in Kubernetes](k8s/resolve-volumemounts-issue-in-kubernetes.md)

## K8s-L1-Cert

***

* [K8s Test](k8s-test.md)
* [Weight: 10](weight-10.md)
* [Weight: 9](weight-9.md)
* [Weight: 8](weight-8.md)
* [Weight: 7](weight-7.md)
* [Weight: 6](weight-6.md)
* [Weight: 5](weight-5.md)
* [Weight: 4](weight-4.md)
* [Weight: 3](weight-3.md)
* [Weight: 2](weight-2.md)
* [Weight: 1](weight-1.md)
* [Weight: 0](weight-0.md)

## Kubernetes Level - 2

* [Kubernetes Shared Volumes](kubernetes-level-2/kubernetes-shared-volumes.md)
* [Kubernetes Sidecar Containers](kubernetes-level-2/kubernetes-sidecar-containers.md)
* [Deploy Nginx Web Server on Kubernetes Cluster](kubernetes-level-2/deploy-nginx-web-server-on-kubernetes-cluster.md)
* [Print Environment Variables](kubernetes-level-2/print-environment-variables.md)
* [Rolling Updates And Rolling Back Deployments in Kubernetes](kubernetes-level-2/rolling-updates-and-rolling-back-deployments-in-kubernetes.md)
* [Deploy Jenkins on Kubernetes](kubernetes-level-2/deploy-jenkins-on-kubernetes.md)
* [Deploy Grafana on Kubernetes Cluster](kubernetes-level-2/deploy-grafana-on-kubernetes-cluster.md)
* [Deploy Tomcat App on Kubernetes](kubernetes-level-2/deploy-tomcat-app-on-kubernetes.md)
